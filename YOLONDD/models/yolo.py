...
from models.BiFormer import *
from models.RIFormer import *
...
from utils.torch_utils import (fuse_conv_and_bn, initialize_weights, model_info, profile, scale_img, select_device,
                               time_sync)
from models.GAM import GAM_Attention
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=4, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0], 'GN') for i in range(2)])
...
class TSCODE_Detect(nn.Module):
    ...
    def __init__(self, nc=4, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.anchor_grid = [torch.empty(0) for _ in range(self.nl)]  # init anchor grid
        self.register_buffer('anchors', torch.tensor(anchors).float().view(self.nl, -1, 2))  # shape(nl,na,2)
        self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0], 'GN') for i in range(4)])
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Decoupled_Detect, TSCODE_Detect, Segment)):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors, nc, gd, gw, act, ch_mul = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple'], d.get('activation'), d.get("channel_muliple")
    if act:
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, SAConv2d, C3_SAC, C3_Res2Block, SPPF_ATT, C3_DySnake, ODConv2d, C3RFEM, C3_DBB, C2f, RepNCSPELAN4}:
            c1, c2 = ch[f], args[0]
            ...
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x,  C3_SAC, C3_Res2Block, C3_DySnake, DySnakeConv, ODConv2d, C3RFEM, C3_DBB, C2f}:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in {ShuffleAttention}:
            args = [ch[f], *args]
        elif m in {CBAMBlock, SpatialAttention, ChannelAttention}:
            c2 = ch[f]
            args = [c2, *args]
            ...
            # TODO: channel, gw, gd
        elif m in {Detect, Decoupled_Detect, TSCODE_Detect, Segment}:
        ...
        elif m in {RIFormer, od_resnet18, od_resnet34, od_resnet50, od_resnet101, od_mobilenetv2_050, od_mobilenetv2_075, od_mobilenetv2_100 }:
            m = m(*args)
            c2 = m.channel
        else:
            c2 = ch[f]
